document.addEventListener("DOMContentLoaded", async () => {
  ...
  try {
    const pages = await Promise.all([1, 2, 3].map((p) => CV.tmdb("/tv/popular", { page: p })));
    const results = pages.flatMap((p) => p.results);
    grid.innerHTML = results.map(CV.cardHTML).join("");
  } catch (e) {
    grid.innerHTML = CV.emptyStateHTML("Couldn't load TV shows", "Check your connection or TMDB key and refresh.");
  }
});
